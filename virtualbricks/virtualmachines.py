...
class VMPlug(Plug, BrickConfig):
    ...
    def hotadd(self):
        ...
        self.brick.send("host_net_add vde sock=%s,vlan=%s\n" %
                        (self.sock.path.rstrip('[]'), self.vlan))
...
class VMSock(Sock, BrickConfig):
    ...
    def __init__(self, brick):
        ...
        self.model = 'rtl8139'
        self.vlan = len(self.brick.plugs) + len(self.brick.socks)
        self.path = MYPATH + "/" + self.brick.name + "_sock_eth" + \
                str(self.vlan) + "[]"
...
class VMDisk():
    ...
    def set_image(self, image):
        ...
        if img:
            ...
            if (not self.cow and self.VM.cfg.get("snapshot") == "" and
                self.image.set_master(self)):
                self.VM.factory.debug("Machine " + self.VM.name +
                                      " acquired master lock on image " +
                                      self.image.name)
            return True
        ...
        if img is None:
            if os.access(image, os.R_OK):
                img = self.VM.factory.new_disk_image(os.path.basename(image),
                                                     image)
        if img is None:
            return False
    ...
    def get_real_disk_name(self):
        ...
        if self.cow:
            ...
            cowname = self.basefolder + "/" + self.VM.name + "_" + \
                    self.device + ".cow"
            if os.access(cowname, os.R_OK):
                ...
                if base != self.get_base():
                    dt = datetime.now()
                    cowback = cowname + ".back-" + \
                            dt.strftime("%Y-%m-%d_%H-%M-%S")
                    self.VM.factory.debug("%s private cow found with a "
                            "different base image (%s): moving it in %s" %
                            (cowname, base, cowback))
                    move(cowname, cowback)
            if not os.access(cowname, os.R_OK):
                qmissing, qfound = self.VM.settings.check_missing_qemupath(
                    self.VM.settings.get("qemupath"))
                if "qemu-img" in qmissing:
                    raise BadConfig(_("qemu-img not found! I can't create a "
                                      "new image."))
                else:
                    self.VM.factory.debug("Creating a new private COW from %s "
                                          "base image." % self.get_base())
                    command = [self.VM.settings.get("qemupath") + "/qemu-img",
                               "create", "-b", self.get_base(), "-f",
                               self.VM.settings.get('cowfmt'), cowname]
                    proc = subprocess.Popen(command, stdout=subprocess.PIPE,
                                            stderr=subprocess.PIPE)
                    proc.wait()
                    proc = subprocess.Popen(["/bin/sync"],
...
class VM(Brick):
    def __init__(self, _factory, _name):
        ...
        self.cfg.snapshot = ""
        self.cfg.boot = ""
        # PRIVATE COW IMAGES MUST BE CREATED IN A DIFFERENT DIRECTORY FOR EACH
        # PROJECT
        self.basepath = self.settings.get("baseimages") + "/." + \
                self.project_parms['id']
        self.cfg.basehda = ""
    ...
    def associate_disk(self):
        for hd in ['hda', 'hdb', 'hdc', 'hdd', 'fda', 'fdb', 'mtdblock']:
            ...
            if hasattr(disk, "image"):
                if (disk.image is not None and self.cfg.get('base' + hd) !=
                    disk.image.name):
    ...
    def prog(self):
        ...
        if self.factory.server:
            try:
                ...
                raise BadConfig(_("KVM not found! Please change VM "
                                  "configuration."))
                return
    ...
    def args(self):
        ...
        if self.factory.nogui is True or self.factory.server is True:
        ...
        for dev in ['hda', 'hdb', 'hdc', 'hdd', 'fda', 'fdb', 'mtdblock']:
            if self.cfg.get("base" + dev) != "":
                ...
                disk.get_real_disk_name()
                if disk.cow is False and disk.readonly() is False:
                    if disk.image.readonly is not True:
                        if disk.image.set_master(disk):
                            self.factory.debug(_("Machine ") + self.name +
                                _(" acquired master lock on image ") +
                                disk.image.name)
                            master = True
                        else:
                            raise DiskLocked(_("Disk image %s already in use."
                                               % disk.image.name))
                            return
                    else:
                        raise DiskLocked(_("Disk image %s is marked as "
                            "readonly and you are not using private cow or "
                            "snapshot mode." % disk.image.name))
                        return
                if self.cfg.get('use_virtio') == "*":
                    res.append('-drive')
                    diskname = disk.get_real_disk_name()
                    res.append('file=' + diskname + ',if=virtio,index=' +
                               str(idx))
                    idx += 1
                else:
        ...
        if (self.cfg.kopt != "" and self.cfg.kernelenbl == "*" and
            self.cfg.kernel != ""):
            res.append("-append")
        ...
        if (len(self.plugs) + len(self.socks) == 0):
            ...
            for pl in sorted(self.plugs + self.socks, key=lambda p: p.vlan):
                res.append("-device")
                res.append("%s,vlan=%d,mac=%s,id=eth%s" % (
                    pl.get_model_driver(), pl.vlan, pl.mac, pl.vlan))
                if (pl.mode == 'vde'):
                    res.append("-net")
                    res.append("vde,vlan=%d,sock=%s" % (
                        pl.vlan, pl.sock.path.rstrip('[]')))
                elif (pl.mode == 'sock'):
                    res.append("-net")
        ...
        if (self.cfg.serial == "*"):
            res.append('-serial')
            res.append('unix:' + MYPATH + '/' + self.name +
                       '_serial,server,nowait')

        res.append("-mon")
        res.append("chardev=mon")
        res.append("-chardev")
        res.append('socket,id=mon_cons,path=%s,server,nowait' %
                   self.console2())
    ...
    def open_internal_console(self):
        ...
        try:
            ...
            if self.proc.stdout is not None:
                self.factory.err(self, "Virtual Machine startup failed. "
